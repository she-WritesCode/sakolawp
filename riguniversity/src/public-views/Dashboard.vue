...
const { enrolledPrograms, fetchEnrolledPrograms, fetchNews, news: events, filter, loading, studentId, fetchHomework, homeworkList } = useDashboardStore();
...
onMounted(() => {
    ...
    fetchNews();
    fetchHomework();
    //   console.log(homeworkList, 'here');
}

);

const navigateToAttendanceReport = () => {
    window.location.pathname = '/attendance_report';
};
    ...
    <StudentLayout>
        ...
        <div class="space-y-10">
            <div class="md:flex justify-between">
                ...
                <div>
                    <Button @click="navigateToAttendanceReport" class="w ring-primary text-red-300 heds" outlined
                        label="Take Attendance"></Button>
                </div>
            ...
            <div class="">
                ...
                <template v-else>
                    <div class="grid lg:grid-cols-3 gap-6">
                        <div v-for="assesement in homeworkList" :key="assesement.homework_id"
                            class="grid grid-cols-5 gap-10 content-center justify-center items-center border-2 bg-white rounded-md p-5">

                            <div class="col-span-4">
                                <h4 class="mb-2 font-semibold text-xl truncate line-clamp-1">{{ assesement.title }}</h4>
                                <div class="font-medium capitalize">assesement</div>
                                <small class="font-semibold text-xs">
                                    <span><strong>Due</strong></span> <span> {{ assesement.due_date }}</span>
                                </small>
                            </div>

                            <div class="col-span-1">
                                <img :src="assesement.submitted == '1' ? done : undone" alt="" />
                            </div>
                        </div>
            ...
            <!-- Meetings -->
            <div class="">
                <h2 class="font-semibold text-xl">Meetings</h2>
                <div class="grid md:grid-cols-2 gap-5">
                    <div v-for="meeting in programMeetings" :key="meeting.ID"
                        ...
                        <div class="col-span-2 md:pl-5 md:col-span-4 lg:flex justify-between items-center w-full">
                            ...
                            <div @click="goToViewProgramMeeting(meeting.ID)"
                                class="flex gap-2 font-semibold text-sm cursor-pointer hover:underline"> View <img
                                    src="./../assets/chevron-right.svg" /></div>
            ...
            <!-- Program Enrolled -->
            <div class="">
                <h2 class="font-semibold text-xl">Programs Enrolled</h2>

                <div class="grid md:grid-cols-2 gap-5">
                    <!-- loader -->
                    <template v-if="loading.enrolledPrograms">
                    ...
                    <template v-else>
                        <div v-for="program in enrolledPrograms" :key="program.id"
                            ...
                            <div class="space-y-3 w-full">
                                ...
                                <div>
                                    <small class="font-semibold text-sm">Started {{
                                        DateHelper.formatMonthsAgo(program.date_added) }}</small>
                                </div>
                            </div>
                        </div>
                    </template>
            ...
            <!-- Latest News -->
            <div class="">
                <h2 class="font-semibold text-xl">Lastest News</h2>
                <div class="grid md:grid-cols-3 gap-5">
                    <div v-for="event in events" :key="event.ID.toString()"
                        class="bg-white border-2 rounded-xl p-2 flex items-center gap-3">
                        <div
                            class="w-[45%] flex flex-col items-center justify-center font-medium bg-black text-white rounded-xl">
                            <img :src="event.img_url.length ? event.img_url[0] : defaultImage" alt=""
                        ...
                        <div class="">
                            ...
                            <h5 class="mb-0 font-semibold truncate line-clamp-2">
                                {{ event.title }}
                            </h5>

                            <div class="text-sm flex w-full justify-between pt-2">
                                <small> {{ DateHelper.relativeTime(event.date) }}</small>
                                <small>RUN News</small>
                            </div>
