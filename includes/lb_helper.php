...
class SakolaLicenseBoxAPI {
	...
	public function __construct(){ 
		$this->product_id = '85BC6DDD';
		$this->api_url = 'https://license.themesawesome.com/';
		$this->api_key = '24B2DF6698DC9E5E3E8B';
		$this->api_language = 'english';
		$this->current_version = 'v1.1.2';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["33af20595fc3af5"])){
				$_SESSION["33af20595fc3af5"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["33af20595fc3af5"])){
				$get_data = $this->call_api(
					'POST',
