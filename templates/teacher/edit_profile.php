...
if (!empty($_POST['action']) && $_POST['action'] == 'update-user') {
	...
	if (!empty(sanitize_text_field($_POST['pass1'])) && !empty(sanitize_text_field($_POST['pass2']))) {
		if (sanitize_text_field($_POST['pass1']) == sanitize_text_field($_POST['pass2']))
			wp_update_user(array('ID' => $current_user->ID, 'user_pass' => esc_attr(sanitize_text_field($_POST['pass1']))));
		else
	...
	if (!empty(sanitize_text_field($_POST['url'])))
		wp_update_user(array('ID' => $current_user->ID, 'user_url' => esc_url(sanitize_text_field($_POST['url']))));
	if (!empty(sanitize_email($_POST['email']))) {
		if (!is_email(esc_attr(sanitize_email($_POST['email']))))
			$error[] = esc_html__('The Email you entered is not valid.  please try again.', 'sakolawp');
		elseif (email_exists(esc_attr(sanitize_text_field($_POST['email']))) != $current_user->ID)
			$error[] = esc_html__('This email is already used by another user.  try a different one.', 'sakolawp');
		else {
			wp_update_user(array('ID' => $current_user->ID, 'user_email' => esc_attr(sanitize_text_field($_POST['email']))));
		}
	}

	if (!empty(sanitize_text_field($_POST['first-name'])))
		update_user_meta($current_user->ID, 'first_name', esc_attr(sanitize_text_field($_POST['first-name'])));
	if (!empty(sanitize_text_field($_POST['last-name'])))
		update_user_meta($current_user->ID, 'last_name', esc_attr(sanitize_text_field($_POST['last-name'])));
	if (!empty(sanitize_text_field($_POST['description'])))
		update_user_meta($current_user->ID, 'description', esc_attr(sanitize_text_field($_POST['description'])));
