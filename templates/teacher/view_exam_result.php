			...
			<div class="info-left info-det skwp-column skwp-column-2">
				<ul>
					...
					<li>
						...
						echo $the_class[0]['name'].'-'.$the_section[0]['name']; ?>
						</span>
					</li>
					<li>
						<span class="title"><?php echo esc_html__( 'Start Date', 'sakolawp' ); ?></span><span class="isi"><?php echo $row['availablefrom'];?></span>
					</li>
					<li>
						<span class="title"><?php echo esc_html__( 'End Date', 'sakolawp' ); ?></span><span class="isi"><?php echo $row['availableto'];?></span>
					</li>
				</ul>
			</div>
			...
			<div class="info-right info-det skwp-column skwp-column-2">
				<ul>
					<li>
						...
						<span class="value">
							<button href="" class="btn btn-primary btn-sm btn-rounded"><?php echo esc_html(round((($cor/$row['questions']) * 100))); ?></button>
						</span>
					</li>
					<li>
					...
					<li>
						...
						<span class="value">
							<button href="" class="btn btn-success btn-sm btn-rounded"><?php echo $row['duration']; ?> <?php echo esc_html__( 'Minutes', 'sakolawp' ); ?></button>
						</span>
					</li>
					<li>
		...
		<div class="analyze-exam analisis-ulangan skwp-row skwp-clearfix">
			<div class="skwp-column skwp-column-2">
				<div class="mark-exam nilai-ulangan">
					<h1 class="mark nilai"><?php echo round((($cor/$row['questions']) * 100));?></h1>
	...
	<div class="skwp-table table-responsive">
		<table class="table table-lightborder list-pertanyaan-ujian">
			...
			<?php if ($number > 0):
				...
				$data = $wpdb->get_results( "SELECT question,question_excerpt,optiona,optionb,optionc,optiond,correct_answer FROM {$wpdb->prefix}sakolawp_questions WHERE question_id = '$ids[$i]'", ARRAY_A );
			...
			?>                    
				<tr>
					...
					<td class="questions">
						<?php
							...
							if(!empty($row3['question_excerpt'])) {
								echo wp_specialchars_decode($row3['question_excerpt']);
							} else {
								echo wp_specialchars_decode($row3['question']);
							}
						?>
					</td>
					<td>
					...
					<td>
						<?php if($row3['correct_answer'] == $ans[$i]):?>
							<a class="btn nc btn-rounded btn-sm btn-primary skwp-btn"><?php echo round($row3['marks'], 2); ?></a>
						<?php endif;?>
						<?php if($row3['correct_answer'] != $ans[$i] || $ans[$i] == '' || $ans[$i] == 'ragu2wae' || $ans[$i] == 'cacauae'):?>
								<a class="btn nc btn-rounded btn-sm btn-danger skwp-btn"><?php echo esc_html__( '0', 'sakolawp' ); ?></a>
							<?php endif;?>
					</td>
				</tr>
