	...
	<div class="skwp-tab-menu">
		<ul class="skwp-tab-wrap">
			<li class="skwp-tab-items">
				<a class="skwp-tab-item" href="<?php echo add_query_arg( 'exam_code', $exam_code, home_url( 'examroom' ) );?>">
					<i class="os-icon picons-thin-icon-thin-0016_bookmarks_reading_book"></i>
					<span><?php echo esc_html__( 'Exam Detail', 'sakolawp' ); ?></span>
				</a>
			</li>
			<li class="skwp-tab-items">
				<a class="skwp-tab-item" href="<?php echo add_query_arg( 'exam_code', $exam_code, home_url( 'exam_questions' ) );?>">
			...
			<li class="skwp-tab-items active">
				<a class="skwp-tab-item" href="<?php echo add_query_arg( 'exam_code', $exam_code, home_url( 'exam_results' ) );?>">
					<i class="os-icon picons-thin-icon-thin-0100_to_do_list_reminder_done"></i>
					<span><?php echo esc_html__( 'Result', 'sakolawp' ); ?></span>
				</a>
			</li>
			<li class="skwp-tab-items">
				<a class="skwp-tab-item" href="<?php echo add_query_arg( 'exam_code', $exam_code, home_url( 'exam_edit' ) );?>">
						...
						?>
							<?php if ($number > 0): ?>
								<tr>
									...
									<td class="text-center">
										<?php 
											if(!empty($row2["point"]) || $row2["point"] != "") {
												if($row2["point"] >= $teach) : ?>
													<a class="btn nc btn-rounded btn-sm btn-success skwp-btn"> <?php echo $row2["point"]; ?></a>
												<?php endif;?>
												<?php if($row2["point"] < $teach): ?>
													<a class="btn nc btn-rounded btn-sm btn-danger skwp-btn"> <?php echo $row2["point"]; ?></a>
												<?php endif;
											}
											else {
									...
									<td class="text-center">
										<a href="<?php echo add_query_arg( array('exam_code' => $exam_code, 'student_id' => $row1['student_id']), home_url( 'view_exam_result' ) );?>" class="btn btn-rounded btn-sm btn-info skwp-btn">
